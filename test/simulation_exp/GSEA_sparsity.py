...
for i in range(6,17,2):
    print(f'n_sparsity={i}')
    path = f"/disk/tanbowen/scKnockPath/data/simulation/sparsity_data/n_sparsity_beta{i}"
    fdr_values = []
    power_values = []
    
    for seed in range(46, 76):
        print(f'seed={seed}')
        fdr, power = GSEA_test(f"{path}/simu_scRNAseq_100pathways_{i}sparsity_seed={seed}.h5ad")
    ...
    fdr_list[str(i)] = fdr_values
    power_list[str(i)] = power_values
    print('===============================================')

fdr_df = pd.DataFrame(fdr_list)
power_df = pd.DataFrame(power_list)
# Create a folder to store results
results_folder = "/disk/tanbowen/scKnockPath/results/simulation_exp/GSEA_sparsity_results"
if not os.path.exists(results_folder):
    os.makedirs(results_folder)
    print("GSEA_sparsity_results folder created!")

# Save results in the new folder
fdr_df.to_csv(os.path.join(results_folder, "GSEA_sparsity_fdr_values.csv"), index=False)
power_df.to_csv(os.path.join(results_folder, "GSEA_sparsity_power_values.csv"), index=False)
