...
folder_name = f"scKnockPath_fdrs_results"
folder_path = os.path.join("./results/simulation_exp", folder_name,'results')
if not os.path.exists(folder_path):
    os.makedirs(folder_path)
    print("文件夹已创建！")
    

target_fdrs = [0.1, 0.2, 0.3]
seeds = np.arange(46, 56)  # 20 datasets
n_overlap = 5
knockoff_seed = 42
...
for fdr in target_fdrs:
    # record the fdr and power for each data seed
    actual_fdrs = []
    actual_powers = []
    for seed in seeds:
        adata = sc.read_h5ad(
        f"./data/simulation/overlap_data/overlap={n_overlap}/simu_scRNAseq_100pathways_{n_overlap}overlap_seed={seed}.h5ad")
        true_effect_pathways = adata.uns["effect_pathways"]
        print(f"data_seed={seed}")
        
        model=pkl.load(open(f"results/simulation_exp/scKnockPath_overlap_results/overlap={n_overlap}/100pathways_{n_overlap}overlap_dataseed={seed}_knockoffseed={knockoff_seed}_copula.pkl",'rb'))
    ...
    results_file_path = os.path.join(
        folder_path, f"fdr={fdr}_results.npz"
    )
