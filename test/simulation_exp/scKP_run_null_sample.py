...
n_samples = np.arange(600, 1801, 240)
...
for n_sample in n_samples:
    ...
    for seed in seeds:
        # load data
        adata = sc.read_h5ad(
            f"./data/simulation/null_sample_data/n_sample_beta{n_sample}/simu_scRNAseq_100pathways_{n_sample}sample_null_seed={seed}.h5ad"
        )
        ...
        load = False
        datafile_name = f"100p_{n_sample}sample_knockoffdata_seed={seed}.npz"
        ...
        model.save_model(
            f"{subfolder_path}/100pathways_{n_sample}sample_null_seed={seed}_lambda.pkl"
        )
        final_selected_pathways = model.select_pathway(fdr=fdr, offset=0)
        n_selected_pathways.append(len(final_selected_pathways))
        print(f"n_selected_pathways={len(final_selected_pathways)}")
        print("-" * 40)
    ...
    results_file_path = os.path.join(
        results_folder_path, f"n_sample={n_sample}_results.npz"
    )
